...
def main():
    ...
    if config.get("tokenizer_config", None):
        ...
        if "model" in tokenizer_config and "vq_model" in tokenizer_config.model:
            ...
            config.model.vq_model = OmegaConf.merge(
                tokenizer_config.model.vq_model,
                config.model.get("vq_model", {})
            )
        else:
    ...
    logger = setup_logger(name="GEN", log_level="INFO",
    ...
    online_tokenization_with_noises = config.training.get("online_tokenization_with_noises", False)
    ...
    # No evaluation on the flight due to the model is sensitive to hparams.
    evaluator = None
    ...
    for current_epoch in range(first_epoch, num_train_epochs):
        ...
        global_step = generator_train_one_epoch(config, logger, accelerator,
